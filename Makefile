...
ifneq ($(abspath $(ROOT_DIR)),$(shell go env GOPATH)/src/github.com/verrazzano/cluster-api-addon-provider-verrazzano)
...
# core
IMAGE_NAME ?= cluster-api-verrazzano-addon-controller
VERRAZZANO_FLEET_CONTROLLER_IMG ?= $(REGISTRY)/$(IMAGE_NAME)
...
docker-build: docker-pull-prerequisites ## Build the docker image for core controller manager
	docker build --build-arg builder_image=$(GO_CONTAINER_IMAGE) --build-arg final_image=$(FINAL_IMAGE) --build-arg goproxy=$(GOPROXY) --build-arg ARCH=$(ARCH) --build-arg ldflags="$(LDFLAGS)" . -t $(VERRAZZANO_FLEET_CONTROLLER_IMG):$(TAG)
	$(MAKE) set-manifest-image MANIFEST_IMG=$(VERRAZZANO_FLEET_CONTROLLER_IMG) MANIFEST_TAG=$(TAG) TARGET_RESOURCE="./config/default/manager_image_patch.yaml"
...
GIT_REPO_NAME ?= cluster-api-addon-provider-verrazzano
GIT_ORG_NAME ?= kubernetes-sigs
USER_FORK ?= $(shell git config --get remote.origin.url | cut -d/ -f4) # only works on https://github.com/<username>/cluster-api-addon-provider-verrazzano.git style URLs
ifdef USER_FORK
USER_FORK := $(shell git config --get remote.origin.url | cut -d: -f2 | cut -d/ -f1) # for git@github.com:<username>/cluster-api-addon-provider-verrazzano.git style URLs
endif
...
release-notes: $(RELEASE_NOTES_DIR) $(RELEASE_NOTES)
	...
	echo ":rotating_light: This is a RELEASE CANDIDATE. Use it only for testing purposes. If you find any bugs, file an [issue](https://github.com/kubernetes-sigs/cluster-api-addon-provider-verrazzano/issues/new)." > $(RELEASE_NOTES_DIR)/release-notes-$(RELEASE_TAG).md; \
	else \
...
docker-push: ## Push the docker images to be included in the release
	docker push $(VERRAZZANO_FLEET_CONTROLLER_IMG):$(TAG)
...
$(CONVERSION_VERIFIER): $(TOOLS_DIR)/go.mod # Build conversion-verifier from tools folder.
	cd $(TOOLS_DIR); go build -tags=tools -o $(BIN_DIR)/$(CONVERSION_VERIFIER_BIN) github.com/verrazzano/cluster-api-addon-provider-verrazzano/hack/tools/conversion-verifier
...
$(RUNTIME_OPENAPI_GEN): $(TOOLS_DIR)/go.mod # Build openapi-gen from tools folder.
	cd $(TOOLS_DIR); go build -tags=tools -o $(BIN_DIR)/$(RUNTIME_OPENAPI_GEN_BIN) github.com/verrazzano/cluster-api-addon-provider-verrazzano/hack/tools/runtime-openapi-gen
...
$(TILT_PREPARE): $(TOOLS_DIR)/go.mod # Build tilt-prepare from tools folder.
	cd $(TOOLS_DIR); go build -tags=tools -o $(BIN_DIR)/tilt-prepare github.com/verrazzano/cluster-api-addon-provider-verrazzano/hack/tools/tilt-prepare
...
## --------------------------------------
## Build Verrazzano Addon Artifacts
## --------------------------------------

VERRAZZANO_ADDON_RELEASE_DIR := release/addon-verrazzano/v${MAJOR_VERSION}.${MINOR_VERSION}.${PATCH_VERSION}

##@ Build Verrazzano Addon  Artifacts:
.PHONY:
verrazzano-addon-build: ## Remove files generated by conversion-gen from the mentioned dirs. Example SRC_DIRS="./api/v1alpha4"
	rm -rf $(VERRAZZANO_ADDON_RELEASE_DIR)
	rm -rf $(RELEASE_DIR)
	mkdir -p $(VERRAZZANO_ADDON_RELEASE_DIR)
	$(MAKE) $(RELEASE_DIR)
	$(MAKE) clean
	$(MAKE) docker-build
	$(MAKE) docker-push
	$(MAKE) release-manifests
	cp out/addon-components.yaml $(VERRAZZANO_ADDON_RELEASE_DIR)/addon-components.yaml
	cp metadata.yaml $(VERRAZZANO_ADDON_RELEASE_DIR)/metadata.yaml
