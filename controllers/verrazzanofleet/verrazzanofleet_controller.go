...
func (r *VerrazzanoFleetReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, options controller.Options) error {
	...
	if err = c.Watch(
		...
		handler.EnqueueRequestsFromMapFunc(VerrazzanoFleetBindingToVerrazzanoFleetMapper),
		predicates.ResourceNotPausedAndHasFilterLabel(log, r.WatchFilterValue),
	); err != nil {
...
// +kubebuilder:rbac:groups=install.verrazzano.io,resources=verrazzanos,verbs=get;list;watch
...
func (r *VerrazzanoFleetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	defer func() {
		...
		if err := patchVerrazzanoFleet(ctx, patchHelper, verrazzanoFleet, r); err != nil && reterr == nil {
			reterr = err
	...
	conditions.MarkTrue(verrazzanoFleet, addonsv1alpha1.VerrazzanoFleetBindingSpecsCreatedOrUpDatedCondition)
...
func (r *VerrazzanoFleetReconciler) reconcileNormal(ctx context.Context, verrazzanoFleet *addonsv1alpha1.VerrazzanoFleet, cluster *clusterv1.Cluster, verrazzanoFleetBindings []addonsv1alpha1.VerrazzanoFleetBinding) error {
	log := ctrl.LoggerFrom(ctx)

	log.V(2).Info("Starting reconcileNormal for verrazzano fleet", "name", verrazzanoFleet.Name)

	err := r.deleteOrphanedVerrazzanoFleetBindings(ctx, verrazzanoFleet, cluster, verrazzanoFleetBindings)
	if err != nil {
		return err
	}
...
// getClustersWithName returns Cluster that match the given name.
func (r *VerrazzanoFleetReconciler) getClustersWithName(ctx context.Context, namespace string, clustername string) (*clusterv1.Cluster, error) {
	log := ctrl.LoggerFrom(ctx)
...
// patchVerrazzanoFleet patches the VerrazzanoFleet object and sets the ReadyCondition as an aggregate of the other condition set.
// TODO: Is this preferrable to client.Update() calls? Based on testing it seems like it avoids race conditions.
func patchVerrazzanoFleet(ctx context.Context, patchHelper *patch.Helper, verrazzanoFleet *addonsv1alpha1.VerrazzanoFleet, reconciler *VerrazzanoFleetReconciler) error {
	log := ctrl.LoggerFrom(ctx)
	vzbinding, err := patchVerrazzanoFleetHelper(ctx, verrazzanoFleet, reconciler)
	if err != nil {
		log.Error(err, "failure to retrieve data for patching")
		return err
	}
	if vzbinding != nil {
		if vzbinding.Status.Status == "Ready" {
...
// VerrazzanoFleetBindingToVerrazzanoFleetMapper is a mapper function that maps a VerrazzanoFleetBinding to the VerrazzanoFleet that owns it.
...
func patchVerrazzanoFleetHelper(ctx context.Context, verrazzanoFleet *addonsv1alpha1.VerrazzanoFleet, reconciler *VerrazzanoFleetReconciler) (*addonsv1alpha1.VerrazzanoFleetBinding, error) {
	log := ctrl.LoggerFrom(ctx)
	cluster, err := reconciler.getClustersWithName(ctx, verrazzanoFleet.Namespace, verrazzanoFleet.Spec.ClusterSelector.Name)
	if err != nil {
		log.Error(err, "unable to get cluster details for fleet patch helper")
		return nil, err
	}
